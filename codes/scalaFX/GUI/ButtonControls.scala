...
val app = new JFXApp {
    stage = new JFXApp.PrimaryStage {
        ...
        scene = new Scene(300, 340) {
            ...
            cb1.selected.onChange(println("Check Box 1 is " + cb1.selected.value))
            cb2.selected.onChange(println("Check Box 2 is " + cb2.selected.value))

            rb1.onAction = (e:ActionEvent) => {
                label.text = "Radibo Button 1 is " + rb1.selected.value
            }
            
            rb2.onAction = (e:ActionEvent) => {
                label.text = "Radibo Button 2 is " + rb2.selected.value
            }
            
            rb3.onAction = (e:ActionEvent) => {
                label.text = "Radibo Button 3 is " + rb3.selected.value
            }

            rb1.selected.onChange(println("Radio button 1 is " + rb1.selected.value))
            rb2.selected.onChange(println("Radio button 2 is " + rb2.selected.value))
            rb3.selected.onChange(println("Radio button 3 is " + rb3.selected.value))
            ...
            tb2.selected.onChange(println("Toggle button 2 is " + tb2.selected.value))
            tb3.selected.onChange(println("Toggle button 3 is " + tb3.selected.value))
        }
    }
}
